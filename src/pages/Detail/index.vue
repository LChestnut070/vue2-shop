      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input v-model="itemNum" type="number" class="itxt" @change="changeNum">
                <button class="plus" @click="itemNum++">+</button>
                <button class="mins" @click="itemNum > 1 ? itemNum-- : itemNum = 1">-</button>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...mapActions('m_detail', ['getGoodsInfo', 'updateCart']),
    // 改变类型选择
    changeActive(AttrValue, AttrValueList, index) {
      // 先遍历全部为不选中
      ...
      // 将选中的属性值赋给checkedVale[当前点击的条目]
      this.checkedValue[index].saleAttrValueName = AttrValue.saleAttrValueName
    },
    // 改变商品数量
    changeNum() {
      if (this.itemNum < 1) {
        ...
        this.itemNum = parseInt(this.itemNum)
      }
    },
    // 添加到购物车
    async addToCart() {
      try {
        ...
        // 将商品类型选择存储到本地
        sessionStorage.setItem('checkedValue', JSON.stringify(this.checkedValue))
        //添加购物车成功，路由跳转
        this.$router.push({
          name: 'Addcartsuccess',
          query: { skuNum: this.itemNum }
        });
      } catch (error) {
        alert("加入购物车失败");
      }
  ...
  created() {
    this.getGoodsInfo(this.skuid);
  },
  computed: {
    ...mapGetters('m_detail', ['categoryView', 'skuInfo', 'spuSaleAttrList']),
    // 商品大图
    imageList() {
        ...
        item.spuSaleAttrValueList.forEach((item2) => {
          if (item2.isChecked == '1') {
            this.checkedValue.push({
              // 默认ischecked为1的为默认值
              saleAttrName: item2.saleAttrName,
